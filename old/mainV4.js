...
class GooglyRenderer {
	...
	drawCosmetics(ctx, wrinklers) {
		// Loop through wrinklers and render cosmetics on top
	...
	drawParticles(ctx, wrinklers) {
		// Render particles on top of wrinklers / cosmetics
		for (let i in wrinklers) {
			let me = wrinklers[i];
			ctx.save();
	...
	drawGooglyEyes(ctx, me, slot) {
		let obj;
		if (slot == 'eyes_icon') {
			obj = GooglyObjects[slot];
		} else {
			obj = me.inventory.slot[slot];
		}
		ctx.save();
...
class GooglyMenu extends GooglyRenderer {
	...
	render(imgArr) {
		...
		this.drawGooglyEyes(ctx, this.wrinkler, 'eyes_icon');

		// this.drawObject(ctx, this.wrinkler, 0);

		this.drawCosmetics(ctx, [this.wrinkler])
	...
	show() {
		...
		this.render(this.getSelectedIcons());
	}
	makeSelector(value) {
...
Game.registerMod("Googly Wrinklers", {
	init:function() {
		...
		// Create wrinkler customizer menu
		...
		Game.DrawWrinklers = GW.codeInjector.mergeFunctions(Game.DrawWrinklers, () => { GW.drawWrinklers(ctx, wrinklers), GW.drawParticles(ctx, wrinklers) });
